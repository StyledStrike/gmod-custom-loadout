...
local cvar_primary_limit = CreateConVar('custom_loadout_primary_limit', '5000', bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_NOTIFY),
	'[Custom Loadout] Limits how much primary ammo is given to players.', 0, 9999)

local cvar_secondary_limit = CreateConVar('custom_loadout_secondary_limit', '50', bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_NOTIFY),
...
function CLoadout:GiveWeapons(ply)
	...
	local max_primary = cvar_primary_limit:GetInt()
	local max_secondary = cvar_secondary_limit:GetInt()
	...
	for _, item in ipairs(items) do
		...
		if success and IsValid(weapon) then
			-- give ammo
			local primary_amount = math.Clamp(item[2], 0, max_primary)
			local secondary_amount = math.Clamp(item[3], 0, max_secondary)

			if primary_amount > 0 and weapon:GetPrimaryAmmoType() ~= -1 then
				ply:GiveAmmo(primary_amount, weapon:GetPrimaryAmmoType())
			end

			if secondary_amount > 0 and weapon:GetSecondaryAmmoType() ~= -1 then
				ply:GiveAmmo(secondary_amount, weapon:GetSecondaryAmmoType())
			end

			if cache.preferred == swep.ClassName then
				-- if this is the prefered weapon by this ply, remember it
				preferred_weapon = swep.ClassName
			end
		end
	end

	-- if the prefered weapon was given...
	if preferred_weapon then
		ply:SelectWeapon(preferred_weapon)
	end
end
...
function CLoadout:ReceiveData(len, ply)
	...
	for _, item in ipairs(loadout.items) do
		...
		if swep then
			table.insert(self.cache[steam_id].items, {
